...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="w-full bg-background dark:bg-background font-sans md:px-10 relative"
      ref={containerRef}
    >
      {/* Decorative background elements */}
      <div className="absolute -top-32 -right-32 w-96 h-96 bg-accent/5 rounded-full blur-3xl -z-10" />
      <div className="absolute -bottom-32 -left-32 w-96 h-96 bg-accent/5 rounded-full blur-3xl -z-10" />
          ...
          {data.map((item, index) => (
            ...
            >
              <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
                ...
                <h3
                  className="hidden md:block text-xl md:pl-20 md:text-5xl font-serif font-bold group-hover:text-accent/80 transition-colors duration-300"
                  style={{ color: "#C79E8E" }}
                >
                  {item.title}
                </h3>
              ...
              <div className="relative pl-20 pr-4 md:pl-4 w-full">
                <h3
                  className="md:hidden block text-2xl mb-4 text-left font-serif font-bold group-hover:text-accent/80 transition-colors duration-300"
                  style={{ color: "#C79E8E" }}
                >
                  {item.title}
                </h3>
